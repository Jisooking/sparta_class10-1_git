...
public class NewBoard : MonoBehaviour
{
    ...
    IEnumerator MoveRoutine(Transform transform, Vector2 target)
    {
        ...
        while (elapsed < 0.6f)  //카드를 세로로 펼치기
        {
            transform.position = Vector3.Lerp(start, row, elapsed / 0.6f);
        ...
        while (elapsed < 0.6f)  //카드를 가로로 펼치기
        {
            transform.position = Vector3.Lerp(row, target, elapsed / 0.6f);
            elapsed += Time.deltaTime;
            yield return null;
        }
